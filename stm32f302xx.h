...
/*	READ THE BLOCK DIAGRAM AS IT WILL GIVE THE CORRECT BUS FOR MOST THINGS
 *
 *
 * The block diagram is mostly correct
 *
 *
 *
 * */
...
#define SRAM1_BASEADDR			0x20000000U //16KB

#define ROM						0x1FFFD800U // 8KB
...
#define TIM4_BASEADDR			( APB1PERIPH_BASE + 0x800 )
...
#define CAN_BASEADDR			( APB1PERIPH_BASE + 0x6400 )
...
#define ADC2_BASEADDR			( AHB3PERIPH_BASE + 0x100 )
#define ADC1_2_BASEADDR			( AHB3PERIPH_BASE + 0x300 )
#define ADC3_BASEADDR			( AHB3PERIPH_BASE + 0x400 )
#define ADC4_BASEADDR			( AHB3PERIPH_BASE + 0x500 )
#define ADC3_4_BASEADDR			( AHB3PERIPH_BASE + 0x700 )
...
typedef struct{
	...
	volatile uint32_t BSRP;				// Offset: 0x18
	volatile uint32_t LCKR;				// Offset: 0x1C
	volatile uint32_t AFR[2];			// Offset: 0x20 - 0x24  [0] Low  [1] High
} GPIO_RegDef_t;
...
// #define I2C3_PCLK_EN()		( RCC->APB1ENR |= ( 1 <<  ) )  // For some reason when i debug the board it says it has I2C3 so idk
...
// #define SPI4_PCLK_EN()			( RCC->APB2ENR |= ( 1 << 15 ) ) // For some reason when i debug the board it says it has SPI3 so idk
...
//#define USART4_PCLK_EN()		( RCC->APB1ENR |= ( 1 << 12 ) )
//#define USART5_PCLK_EN()		( RCC->APB1ENR |= ( 1 << 12 ) )

/*
 * Clock Enable Macros for SYSCRGx Peripherals
 * */
...
// #define I2C3_PCLK_DI()		( RCC->APB1ENR &= ~( 1 <<  ) )  // For some reason when i debug the board it says it has I2C3 so idk
...
// #define SPI4_PCLK_DI()			( RCC->APB2ENR &= ~( 1 << 15 ) ) // For some reason when i debug the board it says it has SPI3 so idk
...
//#define USART4_PCLK_DI()		( RCC->APB1ENR &= ~( 1 << 12 ) )
//#define USART5_PCLK_DI()		( RCC->APB1ENR &= ~( 1 << 12 ) )

/*
 * Clock Disable Macros for SYSCRGx Peripherals
 * */
...
/*
 * IRQ(Interrupt Request) Numbers of STM32F302x MCU
